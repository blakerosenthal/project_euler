...
def longest_path(rows: list) -> list:
    for i in reversed(range(len(rows))):
        if i == len(rows)-1:  # last row base case
            longest = dict(zip(range(len(rows[i])), [[entry] for entry in rows[i]]))

        else:
            new_longest = {}
            for position, value in enumerate(rows[i]):
                left = [value] + longest[position]
                right = [value] + longest[position+1]
                new_longest[position] = left if sum(left) > sum(right) else right
...
if __name__ == '__main__':
    ...
    baby_rows = [list(map(int, row.split(' '))) for row in list(RAW.split('\n'))]
    
    print(sum(longest_path(baby_rows)))
