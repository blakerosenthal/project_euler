...
split_grid = raw_grid.strip().split('\n')
split_grid = [list(map(int, row.split(' '))) for row in split_grid]
matrix = np.array(split_grid)
# print('matrix dims:', matrix.shape)

gp = 0
for row in range(17):
    for col in range(17):
        sub_square = matrix[row:row+4, col:col+4]
        top = sub_square[0, :]
        bottom = sub_square[-1, :]
        left = sub_square[:, 0]
        right = sub_square[:, -1]
        diag1 = sub_square[[0,1,2,3], [0,1,2,3]]
        diag2 = sub_square[[3,2,1,0], [0,1,2,3]]
